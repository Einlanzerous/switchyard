...
- **7.0 — Plan data model + REST (foundations) ✅ (SWY-109, PR #122).** Migration `0023` (`plans` / `plan_revisions` / `plan_criteria` / `plan_reviews` + enums; `comments` gains nullable `plan_revision_id` + `plan_anchor` with a CHECK; `plans` inherits the generic `updated_at` bump trigger; circular FK avoided via a soft `current_revision_id` pointer); `shared/src/schemas/plan.ts`; ticket-rooted submit / revise / get / list / review endpoints; the 5 `plan.*` event types fanned out through `writeEvent` (webhooks + rules, loop-skip intact); revision numbering + plan state machine in `lib/plans.ts`; LCS render-time diff. **Locked-decision realized:** a review `rejected` verdict lands the plan back in a `changes_requested` rework state while emitting the DISTINCT `plan.rejected` event (so 7.2 policy/rules can route "wrong approach" differently from a nit-fix loop). Authz reuses Phase 6 (`assertProjectReadable` 404 reads, `assertProjectRole` write); negative-access matrix gained a 7.0 block. Tests: unit (state machine + diff), integration (submit→review→revise lifecycle, guards, authz). Codegen regenerated `openapi.yaml` + client/mcp `api.types.ts`; `ActivityList` renders the new events. **Out for later milestones (seams left, not built):** plan-anchored comment *creation* (7.1), policy + transition guard (7.2).
