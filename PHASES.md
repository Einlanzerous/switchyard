...
- **IL-2 — Greenfield-end reviewer split ✅.** Pocock-style context drop after `run_greenfield_agent` finishes. The implementing agent that just spent up to 30 tool-use turns is making the "is this done" decision deep in sedimented context; a fresh Anthropic client with system prompt only renders that verdict instead. Shipped in commit `de3567e`: new `verify_greenfield` MCP tool in servo-signal (Claude Opus 4.7 by default, `VERIFY_MODEL` override), and a new `add-greenfield-verify` patcher subcommand that inserts `Build Verify Request → Verify Greenfield → Parse Verification → Verification Passed?` between `Greenfield Complete? (true)` and `Build Greenfield Review Payload`. Verifier-rejected runs short-circuit to `Rejected` with the concerns list as a comment before the (more expensive) High Review fires. Idempotent patcher; composes with `migrate-to-switchyard`.
  - **IL-2 v2 — Verifier-as-subagent with Switchyard MCP (future refactor, not blocking).** Now that Switchyard MCP is live (Phase 5.0 / SWY-36), the verifier can be reframed: spawn a fresh Anthropic session with both servo-signal MCP (for `filesystem_read`) and Switchyard MCP (for `get_ticket` / `comment_on_ticket`) attached, hand it just the ticket key, let it gather its own evidence and post its own verdict. Cleaner separation: verifier owns evidence-gathering instead of being fed pre-extracted strings, and n8n's role shrinks to "kick off the verifier" rather than "extract context, call MCP tool, parse JSON, post comment." Wait until the current implementation has run a few real tickets before refactoring — the existing pre-extraction path is fine and may be cheaper at inference time (less tool-loop overhead).
- **IL-3 — Local-model greenfield path ⏳.** Greenfield agent is hardcoded to the Anthropic SDK (`servo-signal/greenfield.go:416-439`). Refactor behind an `LLMBackend` interface with two implementations: `AnthropicBackend` (current behavior, default) and `OllamaBackend` (OpenAI-compatible `/v1/chat/completions` with `tools`, targets `http://ollama:11434/v1`). Selected via `GREENFIELD_BACKEND` env or `backend` MCP arg. Locks the agent loop's contract (turns, tool schemas, session persistence) so both paths share the persistence + resume path. Hard concerns: (a) Gemma's tool calling is prompt-templated, not native — wrap tool_use JSON parsing defensively and treat malformed calls as recoverable errors the model can retry; (b) per-turn `run_cmd` already runs linters in containers, see IL-3a. *Independent of backend choice*, the agent loop can optionally announce Switchyard MCP (Phase 5.0) as an additional tool surface alongside `list_files` / `read_file` / `write_file` / `run_cmd` — lets the agent read its own ticket, post status updates mid-loop, or self-comment guidance requests. Same Gemma fragility caveat applies: a local model that already struggles to emit one MCP tool surface reliably will struggle with two.
