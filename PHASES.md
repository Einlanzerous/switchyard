...
- **4.8 — Sort tickets by due date, incl. cross-board ⏳ (SWY-44).** List endpoint cursor is hard-coded to `(updated_at, id)`. Add configurable `sort_by` query param (`updated_at` / `due_date` / `created_at` / `priority`), nulls-last for `due_date`, and a sort selector control on list + cross-project board views. Cross-board ordering is the explicit user pain point — Jira doesn't do this naturally and switchyard should.
...
- **4.12 — Less disruptive deploys ⏳ (SWY-54 / 55 / 56).** Current deploys take 30-120s during which the backend returns 503 to the frontend's passthrough. Three sub-tasks:
  - **4.12a — Frontend retry buffer ⏳ (SWY-54).** Bun passthrough retries 503 / connection-refused for 15s; Vue API client honors retryDelay. Masks short deploy bumps invisibly.
  - **4.12b — Startup profile ⏳ (SWY-55).** Instrument `server/src/index.ts` boot phases (env / DB / migrate / scheduler / listen), measure variance, fix the long pole.
  - **4.12c — Migration safety guidelines ⏳ (SWY-56).** Codify the rules (additive only on hot tables, two-PR ladder for destructive changes, batched backfills, etc.) into `docs/migrations.md` + audit the existing 15 migrations.
  - (Blue/green via reverse proxy deferred to Phase 5 — Option A in the deploy plan; build only if 4.12a doesn't cut it.)
