...
- **5.0 — Switchyard MCP server ✅ (SWY-36).** New top-level `mcp/` package. Stdio transport (Claude Desktop, Cline, Claude Code, Gemini CLI install patterns); HTTP transport optional for remote consumers. Hand-curated tool surface:
  ...
  - **Landed in PR #31:** all nine tools wired, in-memory test harness (`InMemoryTransport` + mocked fetch), invariants documented in tool descriptions. SDK pinned at `^1.29.0`. Live `tools/call` round-trip pattern verified during the scaffold phase against the dev backend on :4012; new tools follow the same `projects.ts` template so the wire shape is type-checked against `api.types.ts`. Imperium-loop migration to consume the server tracked separately as IL-* follow-ups in `~/imperium-loop/`.
